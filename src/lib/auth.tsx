...
async function loadProfile(userId: string, userEmail: string | undefined): Promise<UserProfile> {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, email, full_name, avatar_url, cover_url, bio, role, organization_id, skills, certification_links, achievements, experience, phone, address, position, department_id, status, created_at, updated_at, organizations(id, name, subdomain, status)')
    .eq('id', userId)
    .maybeSingle();
  ...
  return {
    ...
    certifications: profile?.certification_links || [],
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateProfile = async (updates: Record<string, unknown>) => {
    if (!supabase || !user) return { error: 'Account service is not configured.' };
  ...
  return (
    <AuthContext.Provider value={{ session, user, profile, loading, refreshProfile, signInWithEmail, signUpWithEmail, signInWithGoogle, offlineAccess, signOut, logout: signOut, updateProfile }}>
      {children}
