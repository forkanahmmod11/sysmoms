...
async function loadProfile(userId: string, userEmail: string | undefined): Promise<UserProfile> {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, email, full_name, avatar_url, cover_url, role, organization_id, skills, certification_links, achievements, experience, phone, address, position, department_id, status, created_at, updated_at, organizations(id, name, subdomain, status)')
    .eq('id', userId)
    .maybeSingle();
