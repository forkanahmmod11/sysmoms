...
export type View = 'overview' | 'subscription' | 'tasks' | 'projects' | 'team' | 'departments' | 'schedule' | 'messages' | 'notices' | 'applications' | 'community' | 'transactions' | 'settings' | 'profile';
...
const navItems: NavItem[] = [
  ...
  { label: 'Departments', icon: UsersRound, view: 'departments' },
  { label: 'Schedule', icon: CalendarDays, view: 'schedule' },
  { label: 'Messages', icon: MessageSquare, view: 'messages' },
  { label: 'Notices', icon: Newspaper, view: 'notices' },
  { label: 'Applications', icon: ClipboardCheck, view: 'applications' },
    ...
    <main className="workspace">
      ...
      <aside className={`sidebar ${mobileMenu ? 'open' : ''}`}>
        ...
        <nav className="side-nav">
          ...
          <button disabled={limited} className={`${view === 'profile' && !profileUserId ? 'active' : ''} ${limited ? 'locked' : ''}`} onClick={() => { if (limited) return; setProfileUserId(profile?.id || null); setView('profile'); setMobileMenu(false); }}>
            <UsersRound size={18} /><span>My profile</span>
          </button>
        </nav>
